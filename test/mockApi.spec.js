...
describe('verifica o usuário', () => {
  test('verifica se o usuário é o tunico', async () => api.fetchURL().then((user) => {
    jest.fn().mockReturnValue(user.gender = 'male');
    jest.fn().mockReturnValue(user.name.first = 'Antônio');
    jest.fn().mockReturnValue(user.name.last = 'Britto');
    jest.fn().mockReturnValue(user.location.country = 'Brazil');
    jest.fn().mockReturnValue(user.email = 'tunico@bol.com.br');
    jest.fn().mockReturnValue(user.login.username = 'tunicao123');
    jest.fn().mockReturnValue(user.login.password = '1234567890');
